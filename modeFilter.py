...
newFileName = 'Results/mode_output_algo_' + filenameSplit[len(filenameSplit)-1]
...
#         kernel = sw.cell_neighbors(img, i, j, d=2)

#         # pprint.pprint(kernel)
        
#         # Finding the 2 furthest values from the mean of the kernel
#         meanKernel = int(np.mean(kernel))
#         # print(meanKernel)
#         max1= kernel[0]
#         max2 = kernel[0]
#         indexMax1 = 0
#         indexMax2 = 0
#         for currIndex in range(len(kernel)):
#             # Check if currVal > max1
#             # print(abs(flatKernel[i] - meanKernel))
#             if abs(kernel[currIndex] - meanKernel) >= abs(max1 - meanKernel):
#                 # Make max2 = max1
#                 max2 = max1
#                 indexMax2 = indexMax1
#                 # Make max1 = currVal
#                 max1 = kernel[currIndex]
#                 indexMax1 = currIndex
#             # if currVal <= max1, check if currVal > max2
#             else:
#                 if abs(kernel[currIndex] - meanKernel) >= abs(max2 - meanKernel):
#                     # Update max2 to new val if it is greater than max2
#                     max2 = kernel[currIndex]
#                     indexMax1 = currIndex
#         # Deleting 2 furthest values
#         # print("max1: " + str(max1) + " index1: " + str(indexMax1) + " max2: " + str(max2) + " index2: " + str(indexMax2) + "\n")
#         newFlatKernel = np.delete(kernel, [indexMax1, indexMax2])
